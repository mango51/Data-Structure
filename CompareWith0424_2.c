...
void main(){
	...
	for(i=0;i<num;i++){
		printf("%d 학생 메모 : ", i+1);
		gets(buf);	// buf에 입력한 문자열 가지고 오기
		size = strlen(buf) + 1; // buf에 입력한 문자열의 길이 + 1 = size
		memo[i] = (char*)malloc(size);	// memo[i]에 size 메모리 크기 할당하기
		if(memo[i]==NULL){	//메모리 주소가 0에 있으면 X >> 메모리 주소 0은 항상 비어있음
			printf("메모리 할당 실패 \n");
			return;
		}
		strcpy(memo[i],buf);	// 입력한 문자열 buf 내용을 memo[i]에 복사하기
	}
	printf("메모 출력\n");
	for(i = 0; i<SN; i++){
		printf("%d번 학생 메모 : %s\n", i+1,memo[i]);
	...
	//pptr = (char**) malloc(num*sizeof(char*));	//char 이중 포인터형에... (char 포인터형 메모리 크기 (4byte) * num (입력한 학생 수)) 크기의 메모리 크기 할당하기
	//if(pptr==NULL){
	//	printf("메모리 할당 실패! \n");
	//	return;
	//}
	//for(i=0;i<num;i++){
	//	printf("%d번째 학생 메모 : ",i+1);
	//	gets(buf);
	//	size = strlen(buf)+1;
	//	pptr[i] = (char*)malloc(size);
	//	if(pptr[i]==NULL){
	//		printf("메모리 할당 실패! \n");
	//		return;
	//	}
	...
	//}
	//for(i=0;i<num;i++){
	//	free(pptr[i]);	// pptr이 가리키는 곳의 메모리 컴퓨터에 반환
	//}
	//free(pptr);	//pptr 메모리 컴퓨터에 반환
}
